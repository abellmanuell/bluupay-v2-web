...
type InfoEntry = { val?: string; color?: string };
type NavInfoMap = Record<string, { info?: InfoEntry }>;
...
export function MenuItem({ data }: { data: NavigationTree }) {
  ...
  invariant(icon && navigationIcons[icon], `[MenuItem] Icon ${icon} not found in navigationIcons`);
  invariant(path, "[MenuItem] path is required but not found");

  const Icon = navigationIcons[icon];
  const label = transKey ? t(transKey) : title;

  // ✅ Read root context (set via createRootRouteWithContext + <RouterProvider context={...} />)
  //    Supports either:
  //    - Context is a NavInfoMap, or
  //    - Context is { navigationInfo: NavInfoMap }
  const rootCtx = useRouteContext({ from: rootRouteId }) as
    | NavInfoMap
    | { navigationInfo?: NavInfoMap }
    | undefined;

  const infoMap: NavInfoMap | undefined =
    rootCtx && typeof rootCtx === "object" && "navigationInfo" in rootCtx
      ? (rootCtx.navigationInfo as NavInfoMap | undefined)
      : (rootCtx as NavInfoMap | undefined);

  const info = infoMap?.[id]?.info;

  // ✅ Active state with useMatchRoute
